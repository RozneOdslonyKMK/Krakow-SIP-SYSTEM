...
try:
    if sys.platform != "win32":
        ...
        print("INFO: Tryb GPS wyłączony na Windows.")
except ImportError:
    print("INFO: Brak biblioteki gps.")
...
class MainSIPLayout(FloatLayout):
    def __init__(self, csv_file, **kwargs):
        ...
        self.ad_files = [os.path.join(ads_dir, f) for f in os.listdir(ads_dir) if f.lower().endswith(('.mp4', '.mkv', '.avi'))]
        self.ad_files.sort()
        ...
        path_parts = SESSION["selected_csv_path"].replace('\\', '/').split('/')
        line_no = path_parts[-2] if len(path_parts) >= 2 else "0"
        ...
        self.date_label = Label(text="", font_size='32sp', font_name=self.ubuntu_font,
    ...
    def show_route_panel(self):
        ...
        for i, stop in enumerate(self.stops):
            ...
            btn = Button(text=f"{i+1}. {stop['Nazwa']}", size_hint_y=None, height=60, background_color=btn_color)
            ...
            with open(anns_path, 'r', encoding='utf-8') as f:
                for line in f:
                    if '|' in line:
                        ...
                        btn = Button(text=title, size_hint_y=None, height=60, background_color=(0.2, 0.2, 0.2, 1))
    ...
    def scroll_news(self, dt):
        ...
        if hasattr(self, 'should_scroll_stop') and self.should_scroll_stop:
            self.lbl_stop.x -= 2
            if self.lbl_stop.right < 309: self.lbl_stop.x = 309 + 1316
        elif hasattr(self, 'lbl_stop'): self.lbl_stop.x = 309
        if hasattr(self, 'should_scroll_dest') and self.should_scroll_dest:
            self.dest_label.x -= 2
            if self.dest_label.right < 428: self.dest_label.x = 428 + 1197
        elif hasattr(self, 'dest_label'): self.dest_label.x = 428

    def next_ad(self, *args):
        if not self.ad_files: return
        if self.ads:
            try:
    ...
    def _rebuild_video_widget(self, dt):
        ...
        self.video_container = StencilView(size_hint=(None, None), size=(1402, 789), pos=(259, self.screen_h - 184 - 789))
        self.ads = Video(source=self.ad_files[self.current_ad_idx], state='play', volume=0,
                         allow_stretch=True, keep_ratio=False, size_hint=(None, None),
                         size=(1402, 789), pos=(259, self.screen_h - 184 - 789))
    ...
    def _force_play(self, dt):
        if self.ads: self.ads.state = 'play'
        self._loading_ad = False
        ...
        if SESSION["mode"] == "Dom":
            if self.is_at_stop:
                if self.current_idx < len(self.stops) - 1:
                    ...
                    self.play_sequence(sekwencja, callback=lambda: self.update_stop_label(next_stop_data['Nazwa']))
            else:
                self.current_idx += 1
                self.is_at_stop = True
    ...
    def play_sequence(self, file_list, callback=None):
        ...
        if not file_list:
            if callback: Clock.schedule_once(lambda dt: callback(), 0.1)
            return
        ...
        if sound:
            ...
            sound.play()
        else: self.play_sequence(file_list, callback)
...
class SIPApp(App):
    ...
    def on_global_key_down(self, instance, key, scancode, codepoint, modifiers):
        if 'ctrl' in modifiers and codepoint == 'q':
